...
unsigned int nStakeTargetSpacing = 5 * 60;
const int64 nChainStartTime = 1404949594; 
...
const string strMessageMagic = "Guinea Signed Message:\n";
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
	...
	// For calculations, minimum of 10 seconds
	if (nActualSpacing < 10)
		nActualSpacing = 10;
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Guinea miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in Guinea
    bool fStrictPayToScriptHash = true; // Always active in Guinea
    ...
    // ppcoin: fees are not collected by miners as in Guinea
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Guinea", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Guinea genesis block";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:guinea genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("0x600cd0495980ed70bf0ee15150eda4fb8be3b8a4bd47912c11086b5c92277fae"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xe2, 0xd8, 0xa3, 0xf5 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("GuineaMiner : proof-of-work not meeting target");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GuineaMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("GuineaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadGuineaMiner(void* parg);

static bool fGenerateGuineas = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void GuineaMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("Guinea-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateGuineas || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateGuineas) && !fProofOfStake)
                return;
        }
        ...
        printf("Running GuineaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadGuineaMiner(void* parg)
{
    ...
    try
    {
        ...
        GuineaMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadGuineaMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadGuineaMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadGuineaMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateGuineas(bool fGenerate, CWallet* pwallet)
{
    fGenerateGuineas = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateGuineas = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d GuineaMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadGuineaMiner, pwallet))
                printf("Error: NewThread(ThreadGuineaMiner) failed\n");
            Sleep(10);
        }
    }
}
